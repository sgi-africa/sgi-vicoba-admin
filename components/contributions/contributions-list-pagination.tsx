...
import { buildContributionsPageHref } from "@/utils/contributions/contributionsListUrl"
...
export function ContributionsListPagination({
  total,
  page,
  limit,
}: {
  ...
  const prevHref = buildContributionsPageHref(sp, page - 1, { limit: limitStr })
  const nextHref = buildContributionsPageHref(sp, page + 1, { limit: limitStr })
        ...
        <div className="flex items-center gap-0.5 px-0.5">
          {pages.map((p) =>
            p === page ? (
              ...
              <Link
                key={p}
                href={buildContributionsPageHref(sp, p, { limit: limitStr })}
                className={cn(
                  pageButtonClass,
