...
import { buildGroupsPageHref } from "@/utils/groups/groupsListUrl"
...
export function GroupsListPagination({
  total,
  page,
  limit,
}: {
  ...
  const prevHref = buildGroupsPageHref(sp, page - 1, { limit: limitStr })
  const nextHref = buildGroupsPageHref(sp, page + 1, { limit: limitStr })
        ...
        <div className="flex items-center gap-0.5 px-0.5">
          {pages.map((p) =>
            p === page ? (
              ...
              <Link
                key={p}
                href={buildGroupsPageHref(sp, p, { limit: limitStr })}
                className={cn(
                  pageButtonClass,
