...
import { buildNotificationsPageHref } from "@/utils/notifications/notificationsListUrl"
import { cn } from "@/lib/utils"
import { pageWindow } from "@/utils/users/pageWindow"

const pageButtonClass =
  "inline-flex size-8 shrink-0 items-center justify-center rounded-md border text-sm font-medium transition-colors"
...
export function NotificationsListPagination({
  ...
  const prevHref = buildNotificationsPageHref(sp, page - 1, { limit: limitStr })
  const nextHref = buildNotificationsPageHref(sp, page + 1, { limit: limitStr })
        ...
        <div className="flex items-center gap-0.5 px-0.5">
          {pages.map((p) =>
            p === page ? (
              ...
              <Link
                key={p}
                href={buildNotificationsPageHref(sp, p, { limit: limitStr })}
                className={cn(
                  pageButtonClass,
